function [spectrum, chats, CritVal, g, xhat, COVcoeff, nc, amplitudes, phases, amplitude_spectrum]=...
...
% Copyright (c) 2019
%Edited by: Alejandro Gonzalez Garcia - York University
% 
...
% 8) amplitudes  - Amplitudes of removed constituents of known forms
% 9) phases      - Phases of removed constituents of known forms
% 10) amplitude_spectrum - Amplitude spectrum
%--------------------------------------------------------------------------
...
amplitudes = [];
phases = [];
for i=1:length(Q(1:4))
    if Q(i) == 1
        index_placeholder = i;
    end
end
if ~isempty(IndexDatum)
    for i=1:length(IndexDatum)
        index_placeholder = index_placeholder + 1;
    end
end
if length(Q) > 4
    if index_placeholder > 0
        coefficients = xhat(index_placeholder+1:end);
        for i=1:2:length(coefficients)-1
            a = coefficients(i);
            b = coefficients(i+1);
...
% Note that you will have to assign an xlim[] boundary when plotting the 
%spectrum. This must be done such that the x values don't begin too close
%to 0.
a_spectrum = chats(1:2:end);
b_spectrum = chats(2:2:end);
amplitude_spectrum = sqrt(a_spectrum.^2 + b_spectrum.^2);
%--------------------------------------------------------------------------
